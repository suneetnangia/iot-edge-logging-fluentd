...
if [ -z "$1" ] ; then
  echo "Existing resource group name is required." && exit 1;
fi
...
# Create IoT Hub
iotHubOutputs=$(az deployment group create -g $resourceGroup --template-file deploy/iotHub.json --query properties.outputs)
iotHubName=$(jq -r .iotHubName.value <<< $iotHubOutputs)

# Create Log Analytics Workspace
logAnalyticsOutputs=$(az deployment group create -g $resourceGroup --template-file deploy/logAnalytics.json --parameters location='eastus'  --query properties.outputs)
...
az iot edge deployment create -d edgeDeviceDeployment  -n $iotHubName  --content deploy/edgedeploymentmanifest.json --target-condition "deviceid='edgeDevice'"

# # Create edge device VM
vmOutputs=$(az deployment group create \
--resource-group $resourceGroup \
--template-file "deploy/edgeVM.json" \
